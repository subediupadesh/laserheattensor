...
def _(mo):
    mo.md(
        ...
        This marimo app loads `Gibbs_<T>K.csv` files and creates an interactive
        8D parallel-coordinate visualization for Co, Cr, Fe, Ni, temperature,
...
def _(mo):
    csv_folder = mo.ui.text(
        value="0_Thermodynamic_Data_Tensor/csv_files/",
        label="CSV folder",
    )
...
def _(io, os, pd):
    ...
    return load_gibbs_data, make_hover_text, safe_numeric
...
def _(
    ...
    if df_plot is None:
        data_status = mo.callout(
            (
                "No valid CSV files were found. Check folder path and filename "
                f"pattern: `Gibbs_<T>K.csv`. Current folder: `{csv_folder.value}`. "
                "Open the `Missing files` section below to see the browser-read error."
            ),
            kind="danger",
        )
    else:
...
def _(mo):
    png_download = mo.callout(
        (
            "Use the Plotly toolbar camera button to download the current figure "
            "as PNG. The figure background is transparent."
        ),
        kind="info",
    )
...
def _(df_parallel, interactive_plot, manual_line_id, mo, pd, variables):
    ...
    if (
        selected_line_id is not None
        and len(df_parallel) > 0
        and selected_line_id in set(df_parallel["Line_ID"])
    ):
        ...
        metric_view = mo.callout(
            (
                "Hover over a colored line to see values in the Plotly tooltip. "
                "Use box/lasso selection on the plot or choose a Line_ID from the "
                "dropdown to print values here."
            ),
            kind="info",
        )
